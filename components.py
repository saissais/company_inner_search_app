...
def display_select_mode():
    ...
    with st.sidebar:
        ...
        # ラジオボタンでモード選択（デフォルト値を明示的に設定）
        selected_mode = st.radio(
            label="利用目的を選択してください",
            options=[ct.ANSWER_MODE_1, ct.ANSWER_MODE_2],
            index=0,  # デフォルトで最初のオプション（社内文書検索）を選択
...
def display_search_llm_response(llm_response):
    """
    「社内文書検索」モードにおけるLLMレスポンスを表示（ページ番号対応版）
    ...
    # LLMからのレスポンスに、ユーザー入力値と関連性の高いドキュメント情報が入って「いない」場合
        ...
        st.error(ct.NO_DOC_MATCH_MESSAGE, icon=ct.WARNING_ICON)
...
def display_contact_llm_response(llm_response):
    """
    「社内問い合わせ」モードにおけるLLMレスポンスを表示（ページ番号対応版）
